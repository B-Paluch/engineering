...
from .models import Comment, CommentRating, Article, ArticleRating, ProcessedPhoto
from .serializers import CommentSerializer, CommentRatingSerializer, ArticleSerializer, ArticleRatingSerializer, \
    UserSerializer, PhotoSerializer
...
class CommentViewSet(viewsets.ModelViewSet):
    ...
    def rate_movie(self, request, pk=None):
        if 'value' in request.data:
            ...
            try:
                rating = CommentRating.objects.get(user=user.id, comment=comment.id)
                rating.value = value
                rating.save()
                serializer = CommentRatingSerializer(rating, many=False)
                response = {'message': 'Zmieniono komentarz.', 'result': serializer.data}
                return Response(response, status=status.HTTP_200_OK)
            except:
                rating = CommentRating.objects.create(user=user, comment=comment.id, value=value)
                serializer = ArticleRatingSerializer(rating, many=False)
                response = {'message': 'Oceniono komentarz.', 'result': serializer.data}
                return Response(response, status=status.HTTP_200_OK)

        else:
            response = {'message': 'Nieprawidłowe wykorzystanie, nie podano oceny komentarza.'}
            return Response(response, status=status.HTTP_400_BAD_REQUEST)
...
@authentication_classes([SessionCsrfExemptAuthentication])
@api_view(['PUT', 'POST'])
def censor_comment(request, id):
    if request.method == 'POST':
        comment = Comment.objects.get(id=id)
        comment.status = True
        comment.save()
        return Response({"message": "Comment is ok! Thats great! It has been activated!"})
    if request.method == 'PUT':
        comment = Comment.objects.get(id=id)
        comment.status = True
        comment.value = request.data['value']
        comment.save()
        return Response({"message": "Comment is profane but it has been changed!"})